...
jest.mock('../../services/postcliService', () => ({
  callPostCli: jest.fn().mockImplementation(() => Promise.resolve('mocked response'))
}));
...
describe('Console Components', () => {
  ...
  describe('ConsoleView', () => {
    ...
    it('renders command and output correctly', () => {
      ...
      expect(screen.getByText(/Console test component mounted/i)).toBeInTheDocument();
    });
  });
  ...
  describe('ConsoleTest', () => {
    ...
    it('handles command test correctly', async () => {
      ...
      expect(screen.getByText(/Command test completed successfully/i)).toBeInTheDocument();
    });
    ...
    it('handles multiline test correctly', () => {
      ...
      expect(screen.getByText(/Multiple line test message:/i)).toBeInTheDocument();
      expect(screen.getByText(/Line 1: Testing console output/i)).toBeInTheDocument();
      expect(screen.getByText(/Line 2: With multiple lines/i)).toBeInTheDocument();
      expect(screen.getByText(/Line 3: And different content/i)).toBeInTheDocument();
    });
    ...
    it('handles error test correctly', () => {
      ...
      expect(screen.getByText(/This is a test error message/i)).toBeInTheDocument();
      expect(screen.getByText(/And another error message/i)).toBeInTheDocument();
      expect(screen.getByText(/Followed by a normal message/i)).toBeInTheDocument();
    });
    ...
    it('handles stress test correctly', async () => {
      ...
      expect(screen.getByText(/Stress test completed successfully/i)).toBeInTheDocument();
    });
  });
});
